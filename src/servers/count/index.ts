...
export default async function getTotalServerCount(data: McssParamsToken):Promise<Response>{
    const { ip, port, token } = data;
    const headers = { headers: { Authorization: `Bearer ${token}` } };
    try {
        const response = await instance.get(`https://${ip}:${port}/api/servers/count`, headers);
